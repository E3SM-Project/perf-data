...
#for ne in 2; do
for ne in 2 3 4 6 10 15 16 18 20 24; do
#for nth in 1 2; do
   ...
   sed -e s/NE/${ne}/ -e s/NMAX/${nmax}/  \
     ...
     srun -n2 -c 8 --gpus-per-node=2  --gpu-bind=map_gpu:0,1  ${whichdir}/$exexx < ${nlname}

   ff=time-thetaxx-ne${ne}-nmax${nmax}.${SLURM_JOBID}
   cp ${nlname} $ff
