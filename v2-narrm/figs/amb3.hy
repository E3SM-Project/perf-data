...
(defn find-files [pattern &optional path]
  (import fnmatch)
  (svifn path ".")
  (sv fnd-files [])
  (for [(, root dirs files) (os.walk path)]
    (for [f files]
      (when (fnmatch.fnmatch f pattern)
        (.append fnd-files (+ root "/" f)))))
  fnd-files)
...
(try
 (do
  ...
  (defclass Subplot []
    (defn --init-- [me]
      (sv me.d {}))
    (defn --call-- [me nr nc spi]
      (sv t (, nr nc spi))
      (unless (in t me.d)
        (assoc me.d t (pl.subplot nr nc spi)))
      (pl.axes (get me.d t))))
...
(if-main
  ...
  (when-inp ["ooa-from-file" {:fname str
                              :xanchor str :xfldno int
                              :yanchor str :yfldno int}]
    (sv (, x y ooas) (ooa-from-file fname xanchor xfldno yanchor yfldno))
    (for [i (range (len x))]
      (prf "{:10.1f} {:10.3e} {}" (get x i) (get y i)
