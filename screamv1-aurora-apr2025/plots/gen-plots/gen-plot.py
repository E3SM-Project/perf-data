...
def plot_data(aurora_timer_data, aurora_timer_data2, frontier_timer_data, pm_gpu_timer_data, pm_cpu_timer_data, plot_type, timing_col, no_title):
    ...
    # DYCORE ONLY
    colors = ['g', 'r', 'k','b', 'm', 'y', 'c']
    markers = ['s','o', 'v', 'd', 'x', '*', 'p', 'h', '^']
    ...
    for timer, data in aurora_timer_data.items():
        ...
        ax.plot(sorted_nodes, sorted_values, linewidth=2,linestyle='-',marker=markers[idx], markerfacecolor='none', color=colors[idx], label=timer_to_label_dict.get(timer, timer))
        idx+=1
    idx=0
    for timer, data in aurora_timer_data2.items():
        ...
        sorted_values = [data["values"][i] for i in sorted_indices]
        print("AURORA2\n",sorted_nodes,sorted_values,"\n")
        ax.plot(sorted_nodes, sorted_values, linewidth=2,linestyle='--',marker=markers[idx], markerfacecolor='none', color=colors[idx])
        idx+=1
    idx=0
    ...
    for timer, data in pm_gpu_timer_data.items():
        ...
        ax.plot(sorted_nodes, sorted_values, linewidth=1,linestyle='--',marker=markers[idx], markerfacecolor='none', color=colors[idx])
        idx+=1
    idx=0
    ...
    if plot_type == "sdpd" or plot_type=="time": first_legend = ax.legend(loc='lower left', fontsize=15)
    ...
    aurora_is_empty2 = all(len(timer_data["nodes"]) == 0 for timer_data in aurora_timer_data2.values())
    ...
    if not aurora_is_empty:
        aurora_line, = ax.plot([],[],linestyle='-',color='k',label='Aurora [Intel]')
        machine_lines.append(aurora_line)
    if not aurora_is_empty2:
        aurora_line2, = ax.plot([],[],linestyle='--',color='k',label='Aurora2')
        machine_lines.append(aurora_line2)
    if not frontier_is_empty:
        frontier_line, = ax.plot([],[],linestyle='-.',color='k',label='Frontier [AMD]')
        machine_lines.append(frontier_line)
    if not pm_gpu_is_empty:
        pm_gpu_line, = ax.plot([],[],linestyle='--',color='k',label='Perlmutter-GPU [NVIDIA]')
        machine_lines.append(pm_gpu_line)
    if not pm_cpu_is_empty:
        pm_cpu_line, = ax.plot([],[],linestyle=':',color='k',label='Perlmutter-CPU [AMD]')
    ...
    ax.set_xscale("log")
    if True or plot_type=="sdpd" or plot_type=="time": ax.set_yscale("log")
    if plot_type == "eff":
    ...
    ax.set_ylim(min(yticks)-3, max(yticks))
    ...
    ax.legend(handles=machine_lines, loc='upper left', fontsize=15)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plot_type", type=str, choices=["sdpd", "time", "eff"], default="sdpd", help="Y-axis on plot. Default is \"sdpd\".")
    ...
    # Convert data to SDPD or efficiency if needed
    if plot_type == "time":
